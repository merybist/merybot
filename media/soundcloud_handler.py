...
@bot.message_handler(func=lambda message: message.from_user.id not in active_chats and re.match(r"(https?://)?(www\.)?(soundcloud\.com/)([a-zA-Z0-9_-]+)", message.text))
def videos(message: Message):
    soundcloud_url_pattern = r"(https?://)?(www\.)?(soundcloud\.com/)([a-zA-Z0-9_-]+)"

    if re.search(soundcloud_url_pattern, message.text):
        url = message.text.strip()
        bot.send_message(message.chat.id, "⏳ Завантажую SoundCloud...")
        ...
        try:
            with open(video_path, 'rb') as video_file:
                bot.send_video(message.chat.id, video_file, caption="🔗 Завантажуй відео тут 👉 @MeryLoadBot", timeout=240)
